...
```bash
python3 .kv-code/skills/kv-code-issue-digest/scripts/collect_issue_digest.py --labels tui exec --window-hours 24
```
...
```text
Use $kv-code-issue-digest to run the Codex issue digest for labels tui and exec over the previous 24 hours.
```

```text
Use $kv-code-issue-digest to run the Codex issue digest for all areas over the past week.
```
...
```bash
python3 .kv-code/skills/kv-code-issue-digest/scripts/collect_issue_digest.py --all-labels --window "past week" --limit-issues 10
```

Run the focused script tests:

```bash
pytest .kv-code/skills/kv-code-issue-digest/scripts/test_collect_issue_digest.py
```
