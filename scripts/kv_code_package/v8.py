"""KV Code V8 artifact overrides for package Cargo builds."""
...
def fetch_codex_v8_artifacts(
    ...
    *,
    ...
    if spec.is_windows:
        raise RuntimeError(
            f"No KV Code V8 release artifacts for target: {spec.target}"
        )
...
def resolved_v8_crate_version() -> str:
    ...
    cargo_lock = tomllib.loads((REPO_ROOT / "kv-code-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def default_cache_root() -> Path:
    return Path(tempfile.gettempdir()) / "kv-code-package"
...
def ensure_valid_artifact(artifact: Path, checksum: str, url: str) -> None:
    ...
    artifact.unlink(missing_ok=True)
    raise RuntimeError(
        f"KV Code V8 artifact {artifact} failed checksum validation."
    )
