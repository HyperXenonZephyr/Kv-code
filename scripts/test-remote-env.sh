...
# Remote-env setup script for kv-code-rs integration tests.
#
# Usage (source-only):
...
setup_remote_env() {
  ...
  codex_binary_path="${REPO_ROOT}/kv-code-rs/target/debug/codex"
  ...
  (
    cd "${REPO_ROOT}/kv-code-rs"
    cargo build -p kv-code-cli --bin codex
  )
