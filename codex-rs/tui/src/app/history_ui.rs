...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in KV Code Desktop.";
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in KV Code Desktop: {err}. Install or launch KV Code Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch KV Code Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch KV Code Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
if ([string]::IsNullOrWhiteSpace($installLocation)) {{
    Write-Error 'KV Code Desktop package is not installed'
    exit 1
}}
...
if (-not (Test-Path $exe)) {{
    Write-Error "KV Code Desktop executable not found at $exe"
    exit 1
}}
if (-not (Test-Path $app)) {{
    Write-Error "KV Code Desktop app bundle not found at $app"
    exit 1
}}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("KV Code Desktop is only available on macOS and Windows".to_string())
}
