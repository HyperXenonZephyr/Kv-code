...
RUNTIME_PKG_NAME = "openai-kv-code-cli-bin"
...
def _installed_codex_path() -> Path:
    try:
        ...
        raise FileNotFoundError(
            ...
            f"with its {RUNTIME_PKG_NAME} dependency, or set CodexConfig.kv-code_bin "
            "explicitly."
        ) from exc
...
def _installed_codex_path_dirs() -> tuple[Path, ...]:
    try:
        from kv_code_cli_bin import bundled_path_dir
...
def resolve_codex_bin(config: "CodexConfig", ops: CodexBinResolverOps) -> Path:
    if config.kv-code_bin is not None:
        codex_bin = Path(config.kv-code_bin)
        if not ops.path_exists(codex_bin):
            raise FileNotFoundError(
                f"Codex binary not found at {codex_bin}. Set CodexConfig.kv-code_bin "
...
class CodexClient:
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            if self.config.kv-code_bin is None:
