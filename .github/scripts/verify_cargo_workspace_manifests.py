...
"""Verify that kv-code-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the kv-code-rs directory naming conventions.
...
MANIFEST_FEATURE_EXCEPTIONS = {
    "kv-code-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "kv-code-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under kv-code-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `kv-code-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `kv-code-rs/<crate>/Cargo.toml` and "
        "`kv-code-rs/utils/<crate>/Cargo.toml`."
    )
    print(
