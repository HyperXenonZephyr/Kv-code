...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_metadata_name_regex() {
    ...
    let dot_codex = repo_root.join(".kv-code");
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".kv-code")).expect("create .codex");
    fs::write(
        vulnerable_root.join(".kv-code").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".kv-code");
