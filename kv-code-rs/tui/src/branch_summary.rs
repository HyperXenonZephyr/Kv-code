...
mod tests {
    ...
    async fn open_pull_request_uses_current_branch_view_first() {
        let runner = FakeRunner::new(vec![response(
            ...
            /*exit_code*/ 0,
            r#"{"number":20252,"url":"https://github.com/HyperXenonZephyr/Kv-code/pull/20252","state":"OPEN"}"#,
        )]);
        ...
        assert_eq!(
            ...
            StatusLinePullRequest {
                number: 20_252,
                url: "https://github.com/HyperXenonZephyr/Kv-code/pull/20252".to_string(),
            }
        );
        ...
        let runner = FakeRunner::new(vec![
            ...
            response(
                &[
                    ...
                    "-H",
                    "Accept: application/vnd.github+json",
                    "repos/HyperXenonZephyr/Kv-code/commits/head-sha/pulls",
                ],
                /*exit_code*/ 0,
                r#"[{"number":20252,"html_url":"https://github.com/HyperXenonZephyr/Kv-code/pull/20252","state":"open"}]"#,
            ),
        ]);
    ...
    fn status_line_pr_view_parser_requires_open_pr() {
        ...
        assert_eq!(
            pull_request_from_view_output(
                r#"{"number":20252,"url":"https://github.com/HyperXenonZephyr/Kv-code/pull/20252","state":"MERGED"}"#
            ),
            None
        );
    }
