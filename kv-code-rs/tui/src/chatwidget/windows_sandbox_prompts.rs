...
impl ChatWidget {
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the KV Code agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/codex/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default KV Code agent sandbox to continue. Set it up to protect your files and control network access."],
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            ...
            lines.push(line![
                "Your organization requires the default sandbox before KV Code can continue."
            ]);
        }
        lines.push(line![
