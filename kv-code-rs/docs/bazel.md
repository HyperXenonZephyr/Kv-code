...
This repository uses Bazel to build the Rust workspace under `kv-code-rs`.
...
- `rules_rs` imports third-party crates from `kv-code-rs/Cargo.toml` and
  `kv-code-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `kv-code-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
...
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/codex \
  ...
  build --config=ci-linux //kv-code-rs/cli:codex
```
