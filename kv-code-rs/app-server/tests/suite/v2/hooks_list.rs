...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".kv-code"))?;
    std::fs::write(
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".kv-code"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".kv-code"), "echo worktree hook")?;
