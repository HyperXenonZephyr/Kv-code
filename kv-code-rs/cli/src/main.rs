...
#[clap(
    ...
    // Keep the legacy binary target available, but present KV Code as the
    // primary user-facing command.
    bin_name = "kv-code",
    override_usage = "kv-code [OPTIONS] [PROMPT]\n       kv-code [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for KV Code.
    Mcp(McpCli),

    /// Manage KV Code plugins.
    Plugin(PluginCli),

    /// Start KV Code as an MCP server (stdio).
    ...
    /// Launch the KV Code desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update KV Code to the latest version.
    Update,

    /// Diagnose local KV Code installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a KV Code-provided sandbox.
    ...
    /// Apply the latest diff produced by KV Code as a `git apply` to your local working tree.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of KV Code.
...
struct UnsupportedSandboxArgs {
    /// Layer $KV_CODE_HOME/<name>.config.toml on top of the base user config.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for KV Code tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating KV Code via `{cmd_str}`...");
    ...
    println!("\nUpdate ran successfully. Please restart KV Code.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`kv-code update` is not available in debug builds. Install a release build of KV Code to use this command."
        );
    }
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the KV Code installation method. Please update manually from https://github.com/HyperXenonZephyr/Kv-code/releases"
            );
        };
        run_update_action(action)
    }
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.api_key.is_some() {
                        ...
                        eprintln!("Usage: echo YOUR_API_KEY | kv-code login --with-api-key");
                        std::process::exit(1);
                    }
                }
            }
...
fn run_workspace_command(cmd: WorkspaceCommand) -> anyhow::Result<()> {
    ...
    if cmd.json {
        ...
        let payload = serde_json::json!({
            ...
            "scan": {
                ...
                "skippedDirectories": scan.skipped_dirs,
                "symlinks": scan.symlinks,
                "bytes": scan.bytes,
                "truncated": scan.truncated,
                "maxFiles": cmd.max_files,
    ...
    println!("KV Code workspace snapshot");
    println!("Path: {}", root.display());
    if let Some(home) = kv_home {
        println!("KV Code home: {}", home.display());
    }
    match git.as_ref() {
        Some(git) => {
            let branch = git.branch.as_deref().unwrap_or("<detached or unknown>");
            println!("Git branch: {branch}");
            println!(
                "Git changes: +{} ~{} -{} R{} ?{} conflicts {}",
                git.added, git.modified, git.deleted, git.renamed, git.untracked, git.conflicts
            );
        }
        None => println!("Git: not available"),
    }
    println!(
        "Files: {} files, {} directories, {} symlinks, {} skipped dirs{}",
        scan.files,
        scan.dirs,
        scan.symlinks,
        scan.skipped_dirs,
    ...
    println!("Size: {}", format_bytes(scan.bytes));
    let top = top_extensions(&scan, cmd.top_extensions);
    if !top.is_empty() {
...
fn scan_workspace(path: &Path, scan: &mut WorkspaceScan, max_files: usize) -> anyhow::Result<()> {
    ...
    for entry in entries.flatten() {
        scan_workspace(&entry.path(), scan, max_files)?;
        if scan.truncated {
            break;
        }
    }
...
fn workspace_git_status(root: &Path) -> Option<WorkspaceGitStatus> {
...
fn top_extensions(scan: &WorkspaceScan, limit: usize) -> Vec<(String, usize)> {
    let mut extensions = scan
        .extensions
        .iter()
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `kv-code mcp`: `kv-code`, `kv-code exec`, `kv-code review`, `kv-code resume`, `kv-code archive`, `kv-code delete`, `kv-code unarchive`, `kv-code fork`, `kv-code mcp`, `kv-code sandbox`, and `kv-code debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("KV Code executable path is not configured"))?;
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". KV Code's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged KV Code local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
/// Build the final `TuiCli` for a `kv-code resume` invocation.
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run kv-code resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run kv-code resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `kv-code app-server proxy`"
        );
    }
