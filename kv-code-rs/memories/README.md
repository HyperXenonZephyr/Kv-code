...
`kv-code-rs/core/src/memories/`.

## Crates

- `kv-code-rs/memories/read` (`codex-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `kv-code-rs/memories/write` (`codex-memories-write`) owns the write path:
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.kv-code/memories/.git` by `codex-git-utils`
