...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let program = codex_utils_cargo_bin::cargo_bin("kv-code-mcp-server")
            .context("should find binary for kv-code-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("kv-code-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `kv-code-mcp-server` child process.
        //
