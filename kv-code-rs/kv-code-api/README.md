...
Typed clients for Codex/OpenAI APIs built on top of the generic transport in `kv-code-client`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `kv-code-api` and `kv-code-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
