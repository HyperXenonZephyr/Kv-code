...
def _resolve_codex_cmd() -> List[str]:
    ...
    raise FileNotFoundError(
        ...
        "  cargo build -p kv-code-cli --release\n"
        "or for debug:\n"
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            proxy_home = WS_ROOT / ".kv-code_proxy_smoke"
    ...
    # 34. WS: policy tamper (.kv-code artifacts) denied
    codex_home = Path(os.environ["USERPROFILE"]) / ".kv-code"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .kv-code\\policy.json"], WS_ROOT)
    add("WS: .kv-code cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .kv-code policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
